...
class NativeChoiceScorer:
    ...
    def score_answer_labels(self, prompt: str, answer_labels: tuple[str, ...]) -> list[float]:
        # ---------------------------------------------------------
        # Score candidate labels with the native loss path used by
        # this repository's decoder-only Transformer.
        # ---------------------------------------------------------
        return [
...
class TransformersChoiceScorer:
    ...
    def score_answer_labels(self, prompt: str, answer_labels: tuple[str, ...]) -> list[float]:
        # ---------------------------------------------------------
        # Batch all answer labels into one Transformers forward pass
        # and average loss only over each candidate label suffix.
        # ---------------------------------------------------------
        ...
        encoded_answers = [
            self.tokenizer(f" {answer_label}", add_special_tokens=False)["input_ids"]
            for answer_label in answer_labels
        ]
        full_token_ids = [[*prompt_ids, *answer_ids] for answer_ids in encoded_answers]
        max_len = max(len(token_ids) for token_ids in full_token_ids)
        pad_token_id = resolve_hf_pad_token_id(tokenizer=self.tokenizer)
        input_rows = [pad_token_row(token_ids=token_ids, max_len=max_len, pad_token_id=pad_token_id) for token_ids in full_token_ids]
...
def score_native_answer_label(
    ...
    # only for the candidate answer label tokens.
    # ---------------------------------------------------------
